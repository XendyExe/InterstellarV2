...
class GetPaintCommand extends StellarCommandsManager_1.BaseCommand {
    ...
    execute() {
        var _a, _b;
        let world = StellarAPI_1.default.Game.getLocalWorld();
        if (!world) {
            __1.default.logMessage("Not in a ship!");
            return;
        }
        let width = world.block_w;
        let height = world.block_h;
        const name = (_a = StellarAPI_1.default.currentShip) === null || _a === void 0 ? void 0 : _a.name;
        const hex = (_b = StellarAPI_1.default.currentShip) === null || _b === void 0 ? void 0 : _b.hex;
        ...
        for (let py = 0; py < height; py++) {
            for (let px = 0; px < width; px++) {
                let color = exports.PAINT_COLORS[StellarAPI_1.default.Game.getColor(px, py)];
        ...
        `;
        __1.default.logMessage("Opened tab!");
    }
}
class PaintCommand extends StellarCommandsManager_1.BaseCommand {
    constructor() {
        super(...arguments);
        this.name = "paint";
        this.alias = [];
...
class PaintHelper {
    constructor() {
        ...
        this.lastIncorrectNumber = 0;
        this.sortMode = -1;
        StellarEventManager_1.default.addTriggerListener(InterstellarEvents_1.TriggerEvent.DRAW_TOP, this.tick.bind(this));
        StellarCommandsManager_1.default.registerCommand(new GetPaintCommand());
        StellarCommandsManager_1.default.registerCommand(new PaintCommand());
        StellarEventManager_1.default.addEventListener(InterstellarEvents_1.SocketCloseEvent, (() => { this.previewMode = false; }).bind(this));
        ...
        this.menufirst = document.getElementById("isqol-paint-first");
        this.menusecond = document.getElementById("isqol-paint-second");
        this.menusecond.style.display = "none";
        this.previewModeButton = document.getElementById("isqol-paint-preview-mode");
        this.incorrectText = document.getElementById("isqol-paint-incorrect");
        this.previewModeButton.onclick = (() => {
            if (this.previewMode)
                return;
            StellarAPI_1.default.UI.showPrompt("Enable preview?", "Preview mode will replace your current ship paint with the finished pixel art, for you to preview the ship's paint. It is not recommended to paint in this mode, since this mode will desync your client. Rejoin to disable this mode.", (() => {
                this.previewMode = true;
            }).bind(this));
        }).bind(this);
        this.menu = document.getElementById("isqol-paint-menu");
        this.painttable = document.getElementById("isqol-paint-table");
        this.sortselect = document.getElementById("isqol-paint-filter-sorter");
        this.offsetxelement = document.getElementById("isqol-paint-ox");
        this.offsetyelement = document.getElementById("isqol-paint-oy");
        document.getElementById("isqol-paint-gravskates").onchange = (e) => {
            let elm = e.target;
            if (elm.checked)
                GravSkates_1.default.enable();
            else
                GravSkates_1.default.disable();
        };
    ...
    inputError(text) {
        StellarAPI_1.default.UI.showPrompt("Failed to import image", text, () => { });
    }
    handleFiles(files) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let source = yield validateImage(file);
            if (typeof source == "string")
                return this.inputError(source);
            this.paint = source;
            this.paintW = this.paint.length;
            this.paintH = this.paint[0].length;
            this.painting = true;
    ...
    tick() {
        let world = StellarAPI_1.default.Game.getLocalWorld();
        if ((!world || !this.painting || this.menuParent.style.display))
            return;
        this.incorrect = 0;
        ...
        for (let x = 0; x < this.paintW; x++) {
            for (let y = 0; y < this.paintH; y++) {
                ...
                let paint = StellarAPI_1.default.Game.getColor(px, py);
                ...
                if (paint != targetPaint) {
                    ...
                    if ((this.filterColor == -1 || this.filterColor == targetPaint)) {
                        ...
                        StellarAPI_1.default.Game.drawText(hex, px + 0.5, py + 0.5, getColor(targetPaint), 10);
                    }
                    this.incorrect += 1;
                    if (this.previewMode) {
                        StellarAPI_1.default.Game.setColor(px, py, targetPaint);
                    }
                }
            }
        }
        for (const data of Object.values(this.colors)) {
            if (data.element == null || data.datacount == null) {
                ...
                let color = exports.PAINT_COLORS[data.color];
                ...
                data.element.onclick = () => {
                    ...
                    if (this.filterColor == data.color) {
                        this.filterColor = -1;
                        (_a = data.element) === null || _a === void 0 ? void 0 : _a.classList.remove("selected");
                        return;
                    }
                    if (this.filterColor != -1) {
                        (_c = (_b = this.colors[this.filterColor]) === null || _b === void 0 ? void 0 : _b.element) === null || _c === void 0 ? void 0 : _c.classList.remove("selected");
                    }
                    this.filterColor = data.color;
                    (_d = data.element) === null || _d === void 0 ? void 0 : _d.classList.add("selected");
                };
            }
...
function findIndex(a) {
    for (let s = 0; s < 255; s++)
        if (Math.abs(a[0] - exports.PAINT_COLORS[s][0]) <= 1 && Math.abs(a[1] - exports.PAINT_COLORS[s][1]) <= 1 && Math.abs(a[2] - exports.PAINT_COLORS[s][2]) <= 1)
            return s;
    return 256;
}
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let x = 0; x < scanner.width; x += 20) {
            ...
            for (let y = 0; y < scanner.height; y += 20) {
                ...
                if (color === 256)
                    return 'Image contains colors that don\'t exist in Dredark color palette. Use DredArt for pixel arts.';
                if (sD[i + 3] !== 255)
                    return 'Image contains transparency. No transparency is allowed. Use DredArt for pixel arts.';
                w.push(color);
            }
            w.reverse();
            result.push(w);
