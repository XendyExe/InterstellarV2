...
impl InterstellarMusic {
    pub fn new(name: String, hash: String, bytes: &Uint8Array, start_time: f64) -> Self {
        ...
        let Some(mut track) = format.default_track() else { throw_str("File has no default track") };
        ...
        // Try all other tracks in sequence
        if decoder.is_err() {
            console_log!("Decoder had error, attempting every track...");
            for test_track in format.tracks() {
                track_id = test_track.id;
                codec_params = test_track.codec_params.clone();
                decoder = symphonia::default::get_codecs().make(&codec_params, &dec_opts);
                if decoder.is_ok() {
                    track = test_track;
                    console_log!("Found ok track: {:?}", test_track.codec_params);
                    break;
                }
                console_log!("Found error track: {:?}", test_track.codec_params);
            }
        }
        let Ok(decoder) = decoder else {
            let err = decoder.err().unwrap();
            for track in format.tracks() {
                console_log!("Track: {:?}", track.codec_params)
            }
            panic!("Failed to create decoder: {}\nCodex: {codec_params:?}", err);
        };
        let Some(sample_rate) = track.codec_params.sample_rate else {throw_str("Track has no sample rate")};
        let Some(frames) = track.codec_params.n_frames else {throw_str("Track has no frame count")};
        ...
        if start_time != 0.0 {
            ...
            current_time = (start_time * sample_rate as f64) as u64;
            format.seek(
                SeekMode::Accurate,
