...
def patch(path):
    with open(base_dir / path / f"js/{dredkit.initgame}.js", "r", encoding="utf-8") as js_file:
        js = js_file.read()

    js = regex_join_game.sub(function_join_game, js)

    with open(base_dir / path / f"js/{dredkit.initgame}.js", "w", encoding="utf-8") as js_file:
