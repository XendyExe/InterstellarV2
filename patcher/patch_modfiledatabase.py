...
def patch(path):
    with open(base_dir / path / f"js/{dredkit.modfiledatabase}.js", "r", encoding="utf-8") as js_file:
        js = js_file.read()

    js = regex_tryimport.sub(function_tryimport, js)

    with open(base_dir / path / f"js/{dredkit.modfiledatabase}.js", "w", encoding="utf-8") as js_file:
