...
regex_socket_send = re.compile(r"send\(([a-zA-Z0-9_$]+)\) {", re.RegexFlag.MULTILINE)
function_socket_send = lambda match: f"send({match[1]})" + "{" + f"{match[1]} = interstellar.patcher.handleMessageSend({match[1]}); if ({match[1]} == null)" + "{ return; }; "

def patch(path):
    with open(base_dir / path / f"js/{dredkit.client_socket}.js", "r", encoding="utf-8") as js_file:
        js = js_file.read()
    ...
    js = regex_socket_send.sub(function_socket_send, js)

    with open(base_dir / path / f"js/{dredkit.client_socket}.js", "w", encoding="utf-8") as js_file:
