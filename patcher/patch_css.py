...
def patch(path):
    ...
    sheet = cssutils.parseFile((base_dir / path / "index.css").absolute())
    print(sheet.cssText)
    ...
    for key in csskit:
        if key not in variables:
            print("missing", key)
    with open(base_dir / "InterstellarFoundational/modded.css", "r", encoding="utf-8") as css_file:
        css = sheet.cssText.decode("utf-8") + "\n" + css_file.read()
    with open(base_dir / path / "index.css", "w", encoding="utf-8") as css_file:
        css_file.write(css)
