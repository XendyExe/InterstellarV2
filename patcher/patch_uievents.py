...
def patch(path):
    patchFile(base_dir / f"{path}/js/{dredkit.objectmenus}.js", patch_objectmenus)
    patchFile(base_dir / f"{path}/js/{dredkit.teammanager}.js", patch_teammanager)
    patchFile(base_dir / f"{path}/js/{dredkit.settings}.js", patch_settings)
    patchFile(base_dir / f"{path}/js/{dredkit.shipyard}.js", patch_shipyard)
    patchFile(base_dir / f"{path}/js/{dredkit.inventory}.js", patch_inventory)
...
shipyard_filters_regex = re.compile(r"(Searcher\(this\.state\.search\);.*?[a-zA-Z0-9_$]+\.forEach\(([a-zA-Z0-9]+) ?=> ?{)", re.MULTILINE | re.DOTALL | re.UNICODE)
...
def patch_shipyard(component: re.Match):
    ...
    if contains_all(body, ['"https://www.patreon.com/cogg"', "shouldComponentUpdate"]):
    ...
    elif contains_all(body, ["this.props.show_ads", ".USER_SETTINGS.preferred_server", "refreshShipList()", "this.props.join_callback"]):
        ...
        body = shipyard_filters_regex.sub(lambda match: f"{match[1]} if (interstellar.patcher.doShipyardFilters({match[2]})) return;", body)
        print(body)
    return f"class {component.group(1)} extends {component.group(2)}.Component " + "{" + body + "}"
