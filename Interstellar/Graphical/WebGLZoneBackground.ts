...
import CloudscapeBackdrop from "./Backdrops/CloudscapeBackdrop";
...
export class WebGLZoneBackground extends ZoneBackground {
    ...
    constructor(config_path: string, config: BackgroundConfig, object_store: string, width: number, height: number, isPixelArt: boolean, internal_name: string | undefined, devpack: boolean) {
        ...
        if (config.fillColor) {
            let color = parseColor(config.fillColor!!);
            this.fill_color = [getR(color), getG(color), getB(color)];
        } else this.fill_color = null;
    }
    logGroup: any[][] = []
    log(...logged: any[]) {
    ...
    override async load() {
        try {
            ...
            if (Interstellar.settingsManager.settings.disableZoneBackgrounds) {
                this.fill_mode = true;
                this.loaded = true;
                this.locked = false;
                return;
            ...
            this.log(`Loaded sprites in ${this.performance.assets - this.performance.start}ms`);
            ...
            this.log(`Loaded textures in ${this.performance.texture - this.performance.assets}ms`);
            await this.loadGL(gpuSprites);
            this.performance.gl = performance.now();
            this.log(`Loaded GL in ${this.performance.gl - this.performance.texture}ms`);
            this.loadedBackdrops = (this.config.backdrops ?? []).map(config => {
                switch (config.type) {
                    case "cloudscape":
                        if (Interstellar.settingsManager.settings.disableComplexGFX) {
                            return null;
                        }
                        return new CloudscapeBackdrop(config, this.width, this.height);
                    default:
                        console.log(config);
                        throw `Unknown backdrop ${config.type}`
                }
            }).filter(e => e !== null);
            this.loadedBackdrops.reverse();
            this.performance.backdrops = performance.now();
            this.log(`Loaded backdrops in ${this.performance.backdrops - this.performance.gl}ms`);
            this.log(`Loaded background in ${this.performance.backdrops - this.performance.start}ms`);
            this.pushLog(`[GLBG] Loaded ${this.config_path} in ${((this.performance.backdrops ?? this.performance.start!!) - this.performance.start!!).toFixed(2)}ms`);
            this.loaded = true;
            this.locked = false;
            ...
            this.pushLog(`[GLBG] Failed to load ${this.config_path} in ${((this.performance.backdrops ?? this.performance.start!!) - this.performance.start!!).toFixed(2)}ms`);
            console.error(e);
        }
    }
    ...
    async loadAssets(): Promise<RawSprite[]> {
        ...
        this.config.sprites.forEach((sprite, index) => {
            if (sprite.high_detail && Interstellar.settingsManager.settings.disableComplexGFX) { return; }
            if (sprite.low_detail && !Interstellar.settingsManager.settings.disableComplexGFX) { return; }
            if (sprite.path) {
        ...
        if (WEBGL_TESTING_WEBSITE) {
            ...
            if (this.devpack) {
                for (let request of requests) {
                    assets[request] = await Devpack.getFile(request);
                }
            } else {
    ...
    async generateTextures(sprites: RawSprite[]): Promise<GPUSprite[]> {
        ...
        for (const sprite of sprites) {
            ...
            gpuSprite.pxo = sprite.config.pxo ?? 0;
            gpuSprite.pyo = sprite.config.pyo ?? 0;
            gpuSprite.pz = sprite.config.pz ?? 1;
        ...
        this.log(`Successfully created ${finalWidth} x ${finalHeight} x ${atlases.length} static background atlas`);
        this.log("Uploaded gpu textures, memory:", mem, "bytes.");
        this.log(`(${(mem / rawMem) * 100}% unpacked of ${rawMem} bytes`);
        return gpuSprites;
    }
    ...
    lastOpacity = 0;
    lastZoomScale = 0;
    override render(defaultFillColor: [number, number, number]) {
        if (this.fill_mode) {
            if (this.fill_color) gl.clearColor(...this.fill_color, 1);
            else gl.clearColor(...defaultFillColor, 1);
            gl.clear(gl.COLOR_BUFFER_BIT);
            gl.clearColor(0, 0, 0, 0);
            return;
        }
        if (!this.loaded || !this.shader || !this.vao) return;
        this.loadedBackdrops.forEach(elm => elm.render());
        gl.useProgram(this.shader);
        const position = Interstellar.patcher.getPlayerPosition();
        const zoom = Interstellar.patcher.zoom;
    ...
    async unload() {
        ...
        while (this.loadedBackdrops.length > 0) this.loadedBackdrops.pop()?.destroy();
