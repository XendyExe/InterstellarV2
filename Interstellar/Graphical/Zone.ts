...
export default class Zone {
    ...
    createZone() {
        ...
        for (let i = 0; i < this.subzones.length; i++) {
            ...
            if (i == this.currentIndex) {
                ...
                InterstellarWebGL.bgProcessors = subzone.bgProcessors;
    ...
    render(fill_color: [number, number, number]) {
        this.subzones.forEach((sub, i) => {
            if (i == this.currentIndex) return;
            sub.background.render(fill_color);
        });
        // Render active background ON TOP.
        this.subzones[this.currentIndex]?.background.render(fill_color);
    }
}