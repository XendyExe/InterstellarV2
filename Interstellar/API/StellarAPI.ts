...
class InterstellarUIAPI {
    ...
    screenRender(callback: CallableFunction) {
        Patcher.graphics.graphics.screen_render(callback);
    }

    drawRect(x: number, y: number, width: number, height: number, color: number) {
        this.translate(x, y, () => { this.drawRectRaw(color, width, height) });
    }

    translate(x: number, y: number, callback: CallableFunction) {
        return Patcher.graphics.graphics.translate(x, y, callback);
    }
    
    drawRectRaw(color: number, width: number, height: number) {
        return Patcher.graphics.graphics.gfx.draw_rect(color, width, height);
    }
...
class InterstellarGameAPI {
    ...
    getCameraWorldX() {
        return Patcher.interstellarInternals.px;
    }
    getCameraWorldY() {
        return Patcher.interstellarInternals.py;
    }
    getPlayerX() {
        const world = this.getLocalWorld();
        return world.entityComponents.local_player_get_x(world.snapProgress);
    }
    getPlayerY() {
        const world = this.getLocalWorld();
        return world.entityComponents.local_player_get_y(world.snapProgress);
    }
    getNavDestination() {
        return Patcher.interstellarInternals.nav_destination;
    }
    getCurrentZone() {
        return Interstellar.canonicalZone;
    }
    leaveShip(): any {
        // @ts-ignore
        window.returnToMenu();
    ...
    getLatestPredictedCommandNumber() {
        return Patcher.worldManager.LATEST_PREDICTED_COMMAND;
    }

    getWorldState(): WorldState {
        return Patcher.interstellarInternals.world_state;
    }
    getLocalShipState(): ShipState | undefined {
        return Patcher.interstellarInternals.world_state[Patcher.interstellarInternals.world_state.current_world];
    }
    getLocalShipPlayers(): Record<string, PlayerState> | undefined {
        return this.getLocalShipState()?.players;
    }
    getCurrentShipID(): number {
        return Patcher.interstellarInternals.world_state.current_world;
    }
    getShipState(id: string): ShipState | undefined {
        return Patcher.interstellarInternals.world_state[id];
    }
    getShipPlayers(id: string) {
        return this.getShipState(id)?.players;
    }
}
...
class InterstellarTelemetryAPI {
    getEventSchema() { return Interstellar.telemetry.eventSchema; }
    getEventState() { return Interstellar.telemetry.eventState; }
    isDisabled() { return Interstellar.telemetry.isDisabled(); }
    connected() { return Interstellar.telemetry.connected; }
}
...
class StellarAPI {
    ...
    Telemetry: InterstellarTelemetryAPI = new InterstellarTelemetryAPI();
    ...
    isCaptain() {
        return (document.querySelector("#team_manager_button") as HTMLButtonElement)!!.style.display != "none" || !this.websocket
    }
    playerName(): string {
