...
import { settingsModels } from "../API/InterstellarScriptingMod";

let fixed_shrekdark = false;
export default class UIEventDispatcher {
    renderSettings(component: Component, elm: VNode) {
        const children = (elm.props.children as VNode[]);
        if (settingsModels.length > 0) {
            children.push(<section>
                <h3>Individual Mod Settings</h3>
                {...settingsModels.map(e => <button onClick={() => { StellarAPI.UI.toggleUI(e.model_name) }}>{e.name}</button>)}
            </section>);
        }
        (new RenderSettingsEvent(component, elm)).dispatch()
    }
    ...
    renderShiplistSidebar(component: Component,elm: VNode) {
        ...
        let extraMods = [];
        if (getComputedStyle(document.querySelector("#top-bar")!!).animation == '0.7s slideTopBarIn') {
