...
class AssetManager {
    ...
    async init() {
        ...
        this.internalMap = Object.fromEntries(Object.keys(this.internal!!).map(key => [key, this.internal!![key]!!.hash]));
        this.assetPaths = Object.keys(GeneratedAssetTree);
    ...
    async getUpdates(): Promise<number[]> {
        const [update, remove] = this.getInternalUpdates();
    ...
    getInternalUpdates(): [string[], string[]] {
        ...
        for (const key of internalKeys) {
            if (!this.assetPaths.includes(key)) {
                remove.push(key);
            }
        }
    ...
    async closeDatabase() {
        this.reference_counter -= 1;
        if (this.reference_counter < 0) this.reference_counter = 0;
        if (this.reference_counter > 0) return;
        this.database?.close();
