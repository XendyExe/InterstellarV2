...
def decompile_drednot(name, path):
    ...
    code: dict = {}
    while len(code) < 10:
        code = json.loads(get_drednot_internals(path))
    ...
    if not os.path.isdir(base_dir / f"cache/{name}"):
        os.mkdir(base_dir / f"cache/{name}")
    if not os.path.isdir(base_dir / f"cache/{name}/js"):
        os.mkdir(base_dir / f"cache/{name}/js")
    with open(base_dir / f"cache/{name}/index.html", "w", encoding="utf-8") as write:
        write.write(drednot_html)
    with open(base_dir / f"cache/{name}/index.css", "w", encoding="utf-8") as write:
        write.write(drednot_style)
    shutil.rmtree(base_dir / f"cache/{name}/js")
    os.mkdir(base_dir / f"cache/{name}/js")
    ...
    for index, func in code.items():
        ...
        with open(base_dir / f"cache/{name}/js/{script_name}.js", "w", encoding="utf-8") as write:
            write.write(prettify_js(func))
    with open(base_dir / f"cache/{name}/index.js", "w", encoding="utf-8") as write:
        write.write(prettify_js(index_script))
    json.dump(tree, open(base_dir / f"cache/{name}/tree.json", "w"), indent=4)


def get_drednot_internals(path) -> str:
    data_dir = (base_dir / "BrowserPersistentData").absolute()
    if os.path.isdir(data_dir):
        shutil.rmtree(data_dir)
    with sync_playwright() as pw:
        extension = (base_dir / "decompiler").absolute()
